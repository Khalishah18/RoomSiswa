...
fun DetailScreen(
    ...
    Scaffold (
        ...
        ItemDetailsBody(
            itemsDetailsUiState = uiState.value,
            onDelete = {
                //Note: if the user rotates the screen very fast, the opperation may get cancelled
                // and the item may not be deleted from database, This is because when config
                // change occurs, the activity will be recreated and the rememberCoroutineScope sill
...
private fun ItemDetailsBody(
    itemsDetailsUiState: ItemDetailsUiState,
    onDelete: () -> Unit,
    modifier: Modifier = Modifier
){
   Column (
       ...
       ItemDetails(
           siswa = itemsDetailsUiState.detailSiswa.toSiswa(), modifier = Modifier.fillMaxWidth()
       )
